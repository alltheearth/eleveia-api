...
"""
Gerenciador centralizado de cache para integração SIGA.

Responsabilidades:
- Gerenciar cache Redis de dados do SIGA
- Implementar estratégia de TTL por tipo de dado
- Fornecer interface única para cache
...
class SigaCacheManager:
    ...
    Hierarquia de Cache:
    - NÍVEL 1: Dados globais (1h) - guardians, students
    - NÍVEL 2: Guardian detalhado (6h)
    - NÍVEL 3: Boletos (30min - volátil)
    - NÍVEL 4: Buscas (15min)
    """
    ...
    def get_or_fetch_guardians(
        ...
        # Armazena no cache
        cache.set(cache_key, data, timeout=cls.TTL_GUARDIANS_GLOBAL)
        logger.info(f"Cached {len(data)} guardians for {cls.TTL_GUARDIANS_GLOBAL}s")
    ...
    def get_or_fetch_students_relations(
        ...
        cache.set(cache_key, data, timeout=cls.TTL_STUDENTS_GLOBAL)
        logger.info(f"Cached {len(data)} students (relations) for {cls.TTL_STUDENTS_GLOBAL}s")
    ...
    def get_or_fetch_students_academic(
        ...
        cache.set(cache_key, data, timeout=cls.TTL_STUDENTS_GLOBAL)
        logger.info(f"Cached {len(data)} students (academic) for {cls.TTL_STUDENTS_GLOBAL}s")
    ...
    def get_or_fetch_guardian_detail(
        ...
        logger.info(f"Caching guardian detail: {cache_key}")
        cache.set(cache_key, guardian_data, timeout=cls.TTL_GUARDIAN_DETAIL)
    ...
    def get_or_set_student_invoices(
        ...
        # SET
        logger.info(f"Caching {len(invoices_data)} invoices for student {student_id}")
        cache.set(cache_key, invoices_data, timeout=cls.TTL_INVOICES)
    ...
    def cache_search_results(
        ...
        logger.info(f"Caching search results: {cache_key} ({len(results)} items)")
        cache.set(cache_key, results, timeout=cls.TTL_SEARCH)
